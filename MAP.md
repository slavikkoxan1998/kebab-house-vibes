...
- Docker: `Dockerfile` (multi-stage, `node:22-alpine`) + `docker-compose.yml`,
  контейнер `kebab_house_vibes`, порт **8792** (ізольовано на `172.17.0.1:8792` для безпеки). Також `.env` додано у `.gitignore`.
...
- **Живий публічний лінк (2026-09-06), поки DNS не додано:**
  **https://slavikkoxan1998.github.io/kebab-house-vibes/** — статичний
  SPA-експорт (`STATIC_BUILD=1` → `nitro: false` + TanStack Start
  `spa: { enabled: true }` пререндерить `_shell.html`, перейменований на
  `index.html`), задеплоєний на гілку `gh-pages` того ж репо. Репо
  зроблено **публічним** (був приватний — GH Pages на приватних репо
  вимагає платного плану). Перевірено вживу: HTTP 200, живий бейдж
  «Nyní otevřeno/zavřeno» рендериться коректно (він рахується повністю
  на клієнті через `Intl`+`setInterval`, працює однаково у статичному
...
- Секція **Burger** (4 позиції) — референс "Kebab Spot" бургерів не мав,
  ...
  розділ нижче.
- Флип-картки меню: стан відкритої картки (`openIndex`) піднято на рівень
  Menu, тому відкриття нової картки автоматично закриває попередню (фікс
  для тач-девайсів).
