...
## 2026-09-18 08:25 CEST — Безпека: ізоляція порту 8792 на 172.17.0.1 та захист .env

- **Ізоляція порту**: `docker-compose.yml` змінено мапінг порту на `172.17.0.1:8792:8792` замість публічного `8792:8792` (вимога безпеки та закриття прямого доступу в обхід Caddy).
- **Gitignore**: додано `.env`, `.env.*` та виключення `!.env.example` до `.gitignore`.
- **Деплой**: контейнер `kebab_house_vibes` перезапущено, веб-трафік маршрутизується через Caddy.
